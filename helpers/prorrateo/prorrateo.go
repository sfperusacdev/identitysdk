...
Este enfoque garantiza un prorrateo correcto y determinista a lo largo del
tiempo, y es independiente del dominio concreto del valor (dinero, kilos,
créditos, etc.).
*/
...
func AllocateOverTime[
	...
	for _, item := range timeline {
		...
		case valueOccur:
			...
			share := item.value.Div(decimal.NewFromInt(int64(n)))
			for _, a := range active {
				a.Total = a.Total.Add(share)
			}
		}
	}
